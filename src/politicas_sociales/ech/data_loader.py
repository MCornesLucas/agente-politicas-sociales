...
tamaño 1 — no hay forma de saber qué variará en un tercer formato hasta
que exista. Si el INE cambia el formato de nuevo, ese es el momento de
extraer la interfaz común, con dos cambios reales como guía.
"""
...
def load_hogares(path: Path) -> pd.DataFrame:
    ...
    `path` es obligatorio a propósito — un default tipo "el .sav más
    reciente de data/" resolvería a 2019 para siempre desde que el INE
    pasó al CSV combinado: un llamado sin argumentos cargaría un año
    viejo en silencio, sin ningún error. El año lo elige siempre quien
    llama: `data/ech_microdatos/{año}/H_*.sav`.
    """
...
def load_hogares_personas_csv(anio: int | str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    `config.PREFERENCIA_METODOLOGIA_HOGARES`, decisión metodológica
    explícita, no elegida en silencio por el código. Fuera de esas tres
...
def load_empleo(anio: int | str) -> pd.DataFrame:
    ...
    recién ahí promediar entre los 12 (panel rotativo mensual, ver nota
    metodológica en config.py). Nunca se pierde de qué mes vino cada fila.
    ...
    contra los datos reales que publicó el INE, no una suposición). Se
    pide a cada archivo solo las columnas de `config.EMPLEO_COLUMNS` que
    de verdad estén presentes en ese archivo puntual, en vez de romper
    toda la carga por columnas que faltan. Las métricas que dependan de
    una columna ausente para el año elegido van a fallar recién ahí, con
    un error claro sobre qué falta — no antes, y no con un dato inventado.
    """
